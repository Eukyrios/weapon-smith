...
SECTIONS = {
  ...
  'mcx-lt': [
    ...
    ('kill-flash', 'Killflash', 'Opened by any of five magnified optics on the list above. It went a long time with a table here and no chip, because the slot was certain from the rules and no recording had caught it open; the M157 Fire Control System was fitted on purpose to film it.'),
    ('tactical-device', 'Tactical device', 'Opened by the Multi-Purpose Tactical Riser. The RM277&rsquo;s four, which also fit all three rails.'),
    ('muzzle', 'Muzzle', 'On the bare rifle and it stays there, even under the Fierce Barrel &mdash; that barrel is drawn with a can already on the front, and neither recording of it shows a muzzle chip, but the game still lets one be fitted. So this is now the one slot on this rifle with a table and no chip on the stage.'),
    ('barrel', 'Barrel', 'Two barrels, neither in the catalogue, and both of them open the side patches this rifle does not otherwise have. The Fierce Barrel opens a heat shield on top of that, which makes it the only attachment on the site to open three slots at once.'),
    ...
    ('left-patch', 'Left patch', 'Not on the bare rifle. Either barrel opens it, which is where the house assumption that patches belong to the gun came apart.'),
    ('upper-patch', 'Upper patch', 'On the bare rifle, and the first upper patch on any weapon here. The five handguard panels, same as the two side patches &mdash; the RM277 has no upper patch to have copied.'),
    ('right-patch', 'Right patch', 'Not on the bare rifle either, and opened by the same two barrels. The same five panels as the other two patch slots.'),
